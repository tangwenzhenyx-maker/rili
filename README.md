...
- 万年历页支持把选中日期的待办保存到网页本地，并通过 iOS 快捷指令写入系统“提醒事项”。快捷指令名称固定为 `万年历添加提醒`，输入是一段 JSON 文本，包含 `title`、`dueDate`、`dueTime`、`due`、`notes`。
...
这个方案只负责保存并显示从 PWA 新增的待办，同时写入系统提醒事项；不读取提醒事项列表，也不同步完成状态。
